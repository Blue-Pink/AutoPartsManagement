...
namespace APM.Services
{
    public class RedisService : IRedisService
    {
        ...
        private static Dictionary<string, string> EntityPrefix => new()
        {
            {nameof(InboundOrder),"RKD"},
            {nameof(OutboundOrder),"CKD"},
        };
        ...
        public string AutoNumber(string entityName, string prefix = "", int digit = 4)
        {
            ...
            var key = $"AutoNumber:{entityName}:{date}";
            ...
            var sequence = count.ToString($"D{digit}");
